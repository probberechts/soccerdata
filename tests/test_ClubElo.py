...
def test_read_by_date(elo: ClubElo):
...
def test_read_by_date_bad_params(elo: ClubElo):
...
def test_read_club_history(elo: ClubElo):
...
def test_read_club_history_max_age(elo: ClubElo):
...
def test_read_club_history_replacement(monkeypatch, tmp_path):
...
def test_read_club_history_bad_params(elo: ClubElo):
    """It should raise an error if the parameters are invalid."""
    with pytest.raises(ValueError):
